...
from hcs_sg_iac.model.entities import (GROUP_NAME_RE, PROTOCOLS, Group,
                                       Member, Rule, RulesFile)
from hcs_sg_iac.model.remote import RemoteCidr, RemoteGroup

_DELETE_NOTE = (" — apply will plan this rule as a delete (remove it in "
...
def import_snapshot(snap: Snapshot) -> ImportedState:
    ...
    for sg in ordered():
        if not GROUP_NAME_RE.fullmatch(sg.name):
            notes.append(f"skip group {sg.name!r} ({sg.id}): name cannot "
                         f"become a config file (must match "
                         f"{GROUP_NAME_RE.pattern})")
            continue
        if sg.name in by_name:
            notes.append(f"skip group {sg.name!r} ({sg.id}): duplicate "
    ...
    for sg in ordered():
        ...
        wanted = {"ingress": [], "egress": []}
        identities = {"ingress": set(), "egress": set()}
        self_rules = 0
        for r in snap.rules.get(sg.id, ()):
            ...
            if r.remote_group_id:
                ...
                if target is None:
                    notes.append(f"skip rule {r.id} of {name!r}: references "
            ...
            if rule.identity() in identities[r.direction]:
                notes.append(f"skip rule {r.id} of {name!r}: duplicate of an "
                             f"imported rule{_DELETE_NOTE}")
                continue
            identities[r.direction].add(rule.identity())
            wanted[r.direction].append(rule)
        if self_rules:
            notes.append(f"{name!r}: {self_rules} self-referential rule(s) "
                         f"not written (the platform re-adds them; the plan "
                         f"preserves them)")
        groups[name] = Group(name=name, description=sg.description,
                             members=tuple(members))
        rules[name] = RulesFile(security_group=name,
                                ingress=tuple(wanted["ingress"]),
                                egress=tuple(wanted["egress"]),
                                ingress_managed=True, egress_managed=True)
