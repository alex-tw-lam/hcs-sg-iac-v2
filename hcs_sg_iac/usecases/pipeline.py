...
def plan_project(load_project, gateway, project, *,
                 sleep=None, notify=None) -> "tuple[ActionList | None, list]":
...
def plan_destroy_project(gateway, name: str, *,
                         sleep=None, notify=None) -> ActionList:
    ...
    while True:
        try:
            return plan_uc.plan_destroy(name,
                                        plan_uc.read_snapshot(gateway, gateway))
        except (QuotaExhausted, CloudThrottled) as e:
            if waits < _MAX_WAITS and apply_uc.wait_for_window(
