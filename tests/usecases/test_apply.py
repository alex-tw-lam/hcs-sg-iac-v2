...
def test_audit_receives_one_record_per_apply():
    ...
    execute(al, sg_writer=gw, rule_writer=None, binder=None, audit=seen.append)
...
def test_budget_exhaustion_waits_for_the_window_and_continues():
    ...
    results = execute(
        al,
        sg_writer=ExhaustOnce(delay=60),
        rule_writer=None,
        binder=None,
        sleep=slept.append,
        notify=notes.append,
...
def test_throttled_then_resume_completes_only_the_remainder():
    ...
    state = DesiredState(
        groups={
            "alpha": Group("alpha", "a", (Member("10.0.1.10"),)),
            "beta": Group("beta", "b", ()),
        },
        rules={
            "alpha": RulesFile(
                "alpha",
                (Rule("ingress", "tcp", "22", RemoteCidr("203.0.113.0/24")),),
                (),
                True,
                False,
            ),
            "beta": RulesFile(
                "beta",
                (Rule("ingress", "tcp", "443", RemoteCidr("203.0.113.0/24")),),
                (),
                True,
                False,
            ),
    ...
    gw.budget = 3
    results = execute(
        plan_state(gw, state), sg_writer=gw, rule_writer=gw, binder=gw
    )
    ...
    al = ActionList(
        actions=(
            ...
            Action(
                ...
                "rule",
                "boom",
                "ingress tcp 22 from cidr:203.0.113.0/24",
                None,
                CreateRule(
                    sg_id="",
                    rule=Rule(
                        "ingress", "tcp", "22", RemoteCidr("203.0.113.0/24")
                    ),
                ),
            ),
            Action(
