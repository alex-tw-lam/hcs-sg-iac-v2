...
def _state_one(name, rules_file):
    return DesiredState(groups={name: Group(name, "d", ())},
...
def test_clears_names_each_managed_empty_direction_with_cloud_rules():
    ...
    gw = _gw_one_sg("web", [
        CloudRule(id="i1", sg_id="sg-web", direction="ingress", protocol="tcp",
                  ports="22", remote_group_id=None, remote_ip_prefix="0.0.0.0/0"),
        CloudRule(id="e1", sg_id="sg-web", direction="egress", protocol="tcp",
                  ports="80", remote_group_id=None, remote_ip_prefix="0.0.0.0/0"),
        CloudRule(id="e2", sg_id="sg-web", direction="egress", protocol=None,
                  ports=None, remote_group_id=None, remote_ip_prefix="10.0.0.0/8")])
    state = _state_one("web", RulesFile("web",
        (Rule("ingress", "tcp", "22", RemoteCidr("0.0.0.0/0")),),
        (), True, True))                   # egress managed + code-empty
    al = plan_state(gw, state)
...
def test_clears_empty_for_drift_on_non_empty_direction():
    ...
    gw = _gw_one_sg("web", [
        CloudRule(id="e1", sg_id="sg-web", direction="egress", protocol="tcp",
                  ports="80", remote_group_id=None, remote_ip_prefix="0.0.0.0/0")])
    state = _state_one("web", RulesFile("web", (),
        (Rule("egress", "tcp", "443", RemoteCidr("0.0.0.0/0")),), True, True))
    al = plan_state(gw, state)
