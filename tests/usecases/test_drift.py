...
def test_every_change_kind_reports_one_line():
    old = Snapshot(
        sgs=(
            CloudSg(id="s-web", name="web", description=""),
...
def test_structured_result_is_liquibase_shaped():
    ...
    old = Snapshot(
        sgs=(CloudSg(id="s1", name="old", description="a"),),
        rules={"s1": [_rule("r1", "s1")]},
        attached={"s1": [CloudNic(port_id="p1", ip="10.0.0.1")]},
    )
    new = Snapshot(
        sgs=(
            CloudSg(id="s1", name="new", description="a"),
            CloudSg(id="s2", name="added", description=""),
        ),
        rules={"s1": [_rule("r1", "s1", ports="443"), _rule("r9", "s1")]},
        attached={},
    )
    result = diff_inventory(old, new)
    assert {"type": "group", "id": "s2", "name": "added"} in result[
        "unexpected"
    ]
    ...
    assert group_chg["differences"] == [
        {"field": "name", "referenceValue": "old", "comparedValue": "new"}
    ]
    assert rule_chg["differences"] == [
        {"field": "ports", "referenceValue": "80", "comparedValue": "443"}
    ]


def test_identical_snapshots_report_no_drift():
    snap = Snapshot(
        sgs=(CloudSg(id="s1", name="web"),),
        rules={"s1": [_rule("r1", "s1")]},
        attached={"s1": [CloudNic(port_id="p1", ip="10.0.0.1")]},
    )
...
def test_duplicate_names_never_confuse_the_diff():
    ...
    old = Snapshot(
        sgs=(
            CloudSg(id="s1", name="dup", description="a"),
