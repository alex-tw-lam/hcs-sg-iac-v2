...
def test_constants_come_from_the_model():
    gs, rs = schema_export.group_file_schema(), schema_export.rules_file_schema()
    assert gs["properties"]["name"]["pattern"] == GROUP_NAME_RE.pattern
    assert rs["properties"]["security_group"]["pattern"] == GROUP_NAME_RE.pattern
...
def test_ports_accept_string_int_and_list():
    one_of = schema_export.rules_file_schema()["properties"]["ingress"][
        "items"]["properties"]["ports"]["oneOf"]
    kinds = {tuple(sorted(o)) for o in one_of}
...
def test_committed_copies_match_a_fresh_export():
    for name, which in (("group-file.schema.json", "group"),
