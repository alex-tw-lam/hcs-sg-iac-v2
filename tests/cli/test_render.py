...
def test_reversed_executed_results_pair_by_key():
    ...
    member_line = next(
        line for line in out.splitlines() if "nic=abc-123" in line
    )
    rule_line = next(
        line for line in out.splitlines() if "rule=ab12cd34" in line
    )
...
def test_unmatched_plan_row_renders_dash_and_new_renders():
    ...
    assert "(new)" in out
    group_line = next(line for line in out.splitlines() if "sg=s1" in line)
...
def test_unknown_quota_left_renders_remaining_unknown():
    out = render_plan(
        _al(), quota=QuotaPlan(needed=8, left=None), dry_run=True
    )
    assert "Quota: 8 calls needed, remaining unknown." in out
    assert "None left" not in out
    assert (
        json.loads(render_json(_al(), quota=QuotaPlan(needed=8, left=None)))[
            "quota"
        ]["left"]
        is None
    )
