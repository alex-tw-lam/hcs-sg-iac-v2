...
def test_dispatch_table_targets_exist_on_vpcclient():
    ...
    missing = [
        name
        for name in sorted(set(_METHODS.values()))
...
def test_cloud_rule_translation_icmp_type_code_not_ports():
    ...
    r = SimpleNamespace(
        ...
        port_range_max=0,
        remote_group_id=None,
        remote_ip_prefix="10.0.113.0/24",
    )
    cr = HuaweiGateway.__new__(HuaweiGateway)._to_cloud_rule(r)
...
def test_huawei_chokepoint_other_service_error_becomes_cloud_error():
    ...
    assert (
        gw.quota_snapshot().effective_limit == 4
    )  # no shrink on plain errors
...
def test_huawei_chokepoint_client_request_429_also_throttles_with_deadline():
    ...
    def raise_429(req):
        raise ClientRequestException(
            ...
            _sdk_error(
                "429",
                "Max retries exceeded with url: /v2.0/"
...
def test_ssl_verify_false_mutes_the_insecure_request_warning():
    ...
    def _caught_build_and_warn():
        with warnings.catch_warnings(record=True) as caught:
            ...
            build_gateway(cfg)
            warnings.warn(
                "Unverified HTTPS request is being made to host 'x'",
                stacklevel=2,
            )  # mimic the SDK layer
            return [
                str(w.message)
...
def _port(pid, ip, hostname="vm-a"):
    return SimpleNamespace(
        ...
        dns_assignment=[{"hostname": hostname, "ip_address": ip}],
    )
...
def test_list_security_groups_follows_marker_through_pages():
    def sg(i):
        return SimpleNamespace(id=f"sg-{i}", name=f"g{i}", description="")

    pages = [
        [sg(i) for i in range(200)],  # full
...
def test_list_rules_follows_marker_through_pages():
    ...
    limiter = FixedWindowLimiter(
        budget=5, window_seconds=300, clock=lambda: 1000.0
    )
    gw = HuaweiGateway(
